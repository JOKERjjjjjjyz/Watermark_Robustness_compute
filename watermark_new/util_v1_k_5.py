...
kk = 6
eta = 20
z_array = np.array([0,30,35,35,30,35,35])
x_array = np.array([0,25,31,31,26,32,30])
zk = np.max(z_array)
...
def f(z,x,a,b):
    ...
    if visit_f[z][x][a][b]:
        return F[z][x][a][b]
    F[z][x][a][b]=f_computation(z,x,a,b)
    visit_f[z][x][a][b]=1
    return F[z][x][a][b]
        ...
        for b in range(2*eta+1):
            ...
            for ak in range(a+1):
                ...
                for bk in range(b+1):
                    ...
                    sum1 += p_ak * p_bk * pp_previous[0][a-ak][b-bk]*(1-f(z_array[k],x_array[k],ak,bk))
                    sum2 += p_ak * p_bk * (pp_previous[0][a-ak][b-bk]*f(z_array[k],x_array[k],ak,bk)+pp_previous[1][a-ak][b-bk]*(1-f(z_array[k],x_array[k],ak,bk)))
            pp[0][a][b] = sum1
            pp[1][a][b] = sum2
    return pp
                    ...
                    for ak in range(a+1):
                        ...
                        for bk in range(b+1):
                            ...
                            for j in range(1,k+1):
                                zk_sum += z_array[j]
                            p_bk = Hyper(zk_sum,z_array[k],b,bk)
                            sum1 += p_ak * p_bk * (1-f(z_array[k-1],x_array[k-1],a-ak,b-bk))*(1-f(z_array[k],x_array[k],ak,bk))
                            sum2 += p_ak * p_bk * ((1-f(z_array[k-1],x_array[k-1],a-ak,b-bk))*f(z_array[k],x_array[k],ak,bk)+f(z_array[k-1],x_array[k-1],a-ak,b-bk)*(1-f(z_array[k],x_array[k],ak,bk)))
                            # sum1 += p_ak * p_bk * (1-f(z_array[k],x_array[k],a,b))*(1-f(z_array[k],x_array[k],ak,bk))
                            # sum2 += p_ak * p_bk * ((1-f(z_array[k],x_array[k],a,b))*f(z_array[k],x_array[k],ak,bk)+f(z_array[k],x_array[k],a,b)*(1-f(z_array[k],x_array[k],ak,bk)))
                    pp[0][a][b] = sum1
                    pp[1][a][b] = sum2
            pp_previous = pp
        else:
