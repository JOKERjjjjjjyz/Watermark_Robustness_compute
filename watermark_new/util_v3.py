...
#         pp_previous = pp
#     return pp
def compute_pp(a, eta, k, sub_HYPER, sub_F, pp_previous, sub_F1=None):
    sum1_list = np.zeros((2 * eta + 1,))
    sum2_list = np.zeros((2 * eta + 1,))
    for b in range(2 * eta + 1):
        sum1 = 0
        sum2 = 0
        for ak in range(a + 1):
        ...
        sum1_list[b] = sum1
        sum2_list[b] = sum2
    return sum1_list, sum2_list

def p_computation_table(k, pp_previous):
    z = z_array[k]
    x = x_array[k]
    pp = np.zeros((2, 2 * eta + 1, 2 * eta + 1))
    zk_sum = sum(z_array[1:k + 1])
    ...
    # 并行执行 a 的计算
    results = Parallel(n_jobs=2 * eta + 1)(delayed(compute_pp)(a, eta, k, sub_HYPER, sub_F, pp_previous, sub_F1) for a in range(2 * eta + 1))

    # 合并计算结果
    for a in range(2 * eta + 1):
        sum1_list, sum2_list = results[a]
        pp[0][a, :] = sum1_list
        pp[1][a, :] = sum2_list
...
def process_file(i):
    ...
    with open(f"./result/{i}_result_parallel.txt", "a") as file:
