...
k = 6
eta = 10
z_array = np.array([0,33,33,33,33,33,33])
x_array = np.array([0,27,27,27,27,27,27])
zk = np.max(z_array)
...
visit_f=np.zeros((k+1,k+1,max(2*eta+1,zk+1),max(2*eta+1,zk+1)),dtype=np.int32)
F=np.zeros((k+1,k+1,max(2*eta+1,zk+1),max(2*eta+1,zk+1)),dtype=np.float32)

def f(z,x,a,b):
    # f as table
    if visit_f[z][x][a][b]:
        return F[z][x][a][b]
    F[z][x][a][b]=f_computation(z_array[z],x_array[x],a,b)
    visit_f[z][x][a][b]=1
    return F[z][x][a][b]
        ...
        for b in range(2*eta+1):
            ...
            for ak in range(a+1):
                ...
                for bk in range(b+1):
                    ...
                    sum1 += p_ak * p_bk * pp_previous[0][a-ak][b-bk]*(1-f(k,k,ak,bk))
                    sum2 += p_ak * p_bk * (pp_previous[0][a-ak][b-bk]*f(k,k,ak,bk)+pp_previous[1][a-ak][b-bk]*(1-f(k,k,ak,bk)))
            pp[0][a][b] = sum1
            pp[1][a][b] = sum2
    return pp
                    ...
                    for ak in range(a+1):
                        ...
                        for bk in range(b+1):
                            ...
                            for j in range(1,k+1):
                                zk_sum += z_array[j]
                            p_bk = Hyper(zk_sum,z_array[k],b,bk)
                            sum1 += p_ak * p_bk * (1-f(k,k,a,b))*(1-f(k,k,ak,bk))
                            sum2 += p_ak * p_bk * ((1-f(k,k,a,b))*f(k,k,ak,bk)+f(k,k,a,b)*(1-f(k,k,ak,bk)))
                    pp[0][a][b] = sum1
                    pp[1][a][b] = sum2
